...
class Near_Miss extends Controller {
	...
	public function archive_all_near_miss(Request $request) {
		...
		$data = [];
			$data['missedPatients'] = MissedPatient::where('is_archive','=',1)->get();
...
public function  softarchive(Request $request)
{
	...
	$getdata->save();
	return redirect()->back()->with(["msg" => '<div class="alert alert-success"> Near Miss archived Successfully.</div>']);
 }
public function  softunarchive(Request $request)
{
   ...
   $getdata->save();
	 ...
	 return redirect()->back()->with(["msg" => '<div class="alert alert-success">Near Miss unarchived Successfully.</div>']);
}
